...
from utlis.optimizer import LRScheduler
from utlis import utils
from torch.utils.data import DataLoader
from dataset.bert_data import CustomBERTDataset
...
def main():
    ...
    parser.add_argument('--max_length', type=int, default=128)  # Set appropriate value for BERT
    ...
    train_data, validation_data, i_vocab_size, t_vocab_size, opt = \
        problem.prepare(opt.max_length, opt.batch_size, opt.device, opt, opt.data_dir)
    ...
    if os.path.exists(opt.output_dir + '/last/models/last_model.pt'):
        ...
        model, global_step = utils.load_checkpoint(last_model_path, opt.device,
                                                   is_eval=False)
    else:
    ...
    for t_step in range(opt.train_step):
        ...
        global_step = train(train_loader, model, opt, global_step,
        ...
        val_loss = validation(val_loader, model, global_step,
